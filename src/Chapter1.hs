...
closestToZero x y =
  let first = abs (0 - x)
      second = abs (0 - y)
  in if first < second then x else y
...
sumLast2 n =
  let lastTwo = (abs n) `mod` 100
      first   = lastTwo `div` 10
      second  = lastTwo `mod` 10
      sum     = first + second
  in sum
...
-}
firstDigit :: Int -> Int
firstDigit n = getFirstDigit (abs n)
  where
        getFirstDigit :: Int -> Int
        getFirstDigit absN =
          let tens = absN `div` 10
          in if tens == 0 then absN else (getFirstDigit tens)
